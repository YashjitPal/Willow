...
/**
 * Many assertions below read source text rather than rendered output, because
 * they pin invariants that never reach the DOM (lock ordering, clamp bounds).
 * That makes them sensitive to file moves, so every path lives in this one map.
 */
const SOURCE = {
  app: path.join(appDir, 'src', 'app', 'App.tsx'),
  ...
  userData: path.join(repoRoot, 'platform', 'auth', 'src', 'use-user-data.ts'),
  dialogFocus: path.join(repoRoot, 'platform', 'core', 'src', 'dialog-focus.ts'),
  feature: (file) => path.join(repoRoot, 'features', 'agent-builder', 'src', file),
};
  ...
  await build({
    stdin: {
      ...
      contents: `
        ...
        import { AgentsWorkspace } from '@willow/agent-builder/AgentsWorkspace';
        import { CollaborationPanel } from '@willow/agent-builder/CollaborationPanel';
        import { WorkflowSecretsPanel } from '@willow/agent-builder/WorkflowSecretsPanel';
        import { DeploymentSecretsSection } from '@willow/agent-builder/DeploymentSecretsSection';
        import { ChatKitDeployPanel } from '@willow/agent-builder/ChatKitDeployPanel';
        import { GovernanceTab } from '@willow/studio/settings/tabs/GovernanceTab';
    ...
    plugins: [
      ...
      // After the boundary mocks, so their specifiers win over the alias map.
...
it('renders the Agents home with an inert prompt and workflow entry points', () => {
  ...
  // The saved-workflow list used to sit under a "Your agents" heading; it is now
...
it('keeps Agent Builder reachable from app navigation and a direct route', () => {
  const appSource = fs.readFileSync(SOURCE.app, 'utf8');
  const sidebarSource = fs.readFileSync(SOURCE.sidebar, 'utf8');
  const agentBuilderSource = fs.readFileSync(SOURCE.feature('AgentBuilder.tsx'), 'utf8');
  const nodeConfigSource = fs.readFileSync(SOURCE.feature('NodeConfigPanel.tsx'), 'utf8');
  const chatPreviewSource = fs.readFileSync(SOURCE.feature('ChatPreviewPanel.tsx'), 'utf8');
  const runPanelSource = fs.readFileSync(SOURCE.feature('RunPanel.tsx'), 'utf8');
  const runHistorySource = fs.readFileSync(SOURCE.feature('RunHistoryPanel.tsx'), 'utf8');
  ...
  const variablePickerSource = fs.readFileSync(SOURCE.feature('VariablePicker.tsx'), 'utf8');
  const deployPanelSource = fs.readFileSync(SOURCE.feature('ChatKitDeployPanel.tsx'), 'utf8');
  const batchPanelSource = fs.readFileSync(SOURCE.feature('BatchRunPanel.tsx'), 'utf8');
  const backendHookSource = fs.readFileSync(SOURCE.feature('use-agent-builder-backend.ts'), 'utf8');
  ...
  // The dev server must be predictable for QA: a fixed port that fails loudly
  // rather than drifting, and no browser launched out from under the harness.
  // (An `hmr: false` assertion used to live here, but the config has never set
  // it — HMR stays on for normal development.)
  ...
  const dialogFocusSource = fs.readFileSync(SOURCE.dialogFocus, 'utf8');
...
it('keeps the QA user-data dependency identity stable across renders', async () => {
  const module = await import(`${pathToFileURL(SOURCE.qaUserData).href}?v=${Date.now()}`);

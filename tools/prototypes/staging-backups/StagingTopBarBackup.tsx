...
const TopBar: React.FC<TopBarProps> = ({ isSidebarCollapsed, onToggleSidebar }) => {
  ...
  return (
    <div className={`h-14 flex items-center justify-between w-full flex-shrink-0 bg-[#1c1c1c] pr-4 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'pl-4' : 'pl-0'}`}>
      {/* Left Group */}
        ...
        {visibleTools.map((item) => {
          ...
          return (
            <button
              ...
              style={{ 
                minWidth: isLeaving ? '0px' : (isActive ? '112px' : '40px'),
                maxWidth: isLeaving ? '0px' : (isActive ? '112px' : '40px'),
              ...
              className={`
                group flex items-center justify-center h-10 rounded-full text-sm font-medium transition-all duration-300 ease-in-out border overflow-hidden
                ${isActive 
      ...
      {/* Center Address Bar */}
      <div className="flex-1 max-w-xl pl-2 pr-24">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Rainbow Publish Button */}
        <RainbowButton className="h-10 text-sm rounded-full">
          Publish
        </RainbowButton>
      </div>
    </div>

...
const TITLES: Record<string, string> = {
  start: 'Start', end: 'End', note: 'Note', fileSearch: 'File search',
  mcp: 'MCP', ifElse: 'If / else', while: 'While', userApproval: 'User approval',
...
export const NodeConfigPanel: React.FC<Props> = ({ nodeType, nodeName, config, onNameChange, onConfigChange }) => {
  ...
  return (
    <motion.div
      {...panelMotion}
      className="absolute right-6 top-6 w-[350px] bg-[#1a1a1a] rounded-[20px] shadow-2xl flex flex-col z-20 pointer-events-auto border border-[#2b2b2b]"
    ...
    >
      <div className="p-5 pb-8 flex-1 flex flex-col gap-4 min-h-0 overflow-y-auto [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }}>
        ...
        {nodeType === 'ifElse' && <IfElseFields config={config} set={set} />}
        {nodeType === 'while' && <WhileFields config={config} set={set} />}
        {nodeType === 'transform' && <TransformFields config={config} set={set} />}
        {nodeType === 'setState' && <SetStateFields config={config} set={set} />}
        {nodeType === 'userApproval' && <UserApprovalFields config={config} set={set} />}
        {nodeType === 'fileSearch' && <FileSearchFields config={config} set={set} />}
...
const IfElseFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => {
  ...
  return (
    <Row>
      <span className={label}>Condition (CEL)</span>
      <textarea
        rows={3}
        className={textareaCls}
        placeholder={'e.g. classifier.output_parsed.category == "billing"'}
        value={first.condition ?? ''}
        onChange={(e) => set({ branches: [{ id: 'if', label: first.label ?? 'If', condition: e.target.value }] })}
      />
...
const WhileFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => (
  <>
    <Row>
      <span className={label}>Condition (CEL)</span>
    ...
    <div className="flex items-center justify-between gap-3">
      ...
      <input type="number" className="w-24 bg-[#2b2b2b] rounded-lg px-3 h-9 text-white text-[13px] outline-none" value={config.maxIterations ?? 100} onChange={(e) => set({ maxIterations: Number(e.target.value) || 100 })} />
    </div>
    <div className="flex items-center justify-between gap-3">
...
const TransformFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => {
    ...
    <div className="flex flex-col gap-2.5">
      ...
      {outs.map((o, i) => (
        <div key={i} className="flex flex-col gap-1.5 bg-[#222] rounded-lg p-2.5">
          ...
          <input className="w-full bg-[#2b2b2b] rounded-lg px-2.5 h-8 text-white text-[12.5px] outline-none font-mono" placeholder="CEL expression" value={o.expression ?? ''} onChange={(e) => update(i, { expression: e.target.value })} />
        </div>
      ))}
...
const SetStateFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      {asg.map((a, i) => (
        <div key={i} className="flex flex-col gap-1.5 bg-[#222] rounded-lg p-2.5">
          <div className="flex items-center gap-1.5">
            <input className="flex-1 bg-[#2b2b2b] rounded-lg px-2.5 h-8 text-white text-[12.5px] outline-none" placeholder="state variable" value={a.name ?? ''} onChange={(e) => update(i, { name: e.target.value })} />
            <button className="text-[#6a6a6a] hover:text-red-400" onClick={() => set({ assignments: asg.filter((_, idx) => idx !== i) })}><Trash2 size={14} /></button>
          </div>
          <input className="w-full bg-[#2b2b2b] rounded-lg px-2.5 h-8 text-white text-[12.5px] outline-none font-mono" placeholder="CEL expression, e.g. state.i + 1" value={a.expression ?? ''} onChange={(e) => update(i, { expression: e.target.value })} />
        </div>
      ))}
...
const UserApprovalFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => (
  <Row>
    <span className={label}>Approval message</span>
...
const FileSearchFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => {
...
const EndFields: React.FC<{ config: Cfg; set: (p: Cfg) => void }> = ({ config, set }) => (
  <Row>

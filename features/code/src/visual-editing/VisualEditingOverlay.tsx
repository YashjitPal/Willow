...
import { AIInputWithLoading } from '@willow/ui/ai-input-with-loading';
...
const VisualEditingOverlay: React.FC<VisualEditingOverlayProps> = ({ iframeRef, selectedModelId, modelConfig, isReloading = false }) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (e: MessageEvent) => {
      ...
      if (e.data?.type === 'THEME_COLORS_RESPONSE' && e.data.colors) {
        import('./engine/visual-editor-store').then(({ themeColors }) => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (el) {
      ...
      import('./engine/index').then(({ setHoveredElement }) => {
        setHoveredElement({
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    if (elements.length > 0) {
      ...
      import('./engine/index').then(({ setSelectedElements }) => {
        ...
        familyElementsRef.current = [];
        setIsPromptVisible(false);
        import('./engine/index').then(({ clearSelection }) => {
            clearSelection();
        });
    }
      ...
      if (!result.success) {
        ...
        setTimeout(() => {
          if (!iframeRef.current?.contentDocument || !currentElement?.sourceLocation) {
            ...
            import('./engine/index').then(({ clearSelection }) => clearSelection());
            return;
          }
          ...
          if (el) {
            ...
            import('./engine/index').then(({ setSelectedElements: setGlobalSelected }) => {
              setGlobalSelected([{
                uid: newUid,
                tagName: el!.tagName.toLowerCase(),
                classNames: Array.from(el!.classList),

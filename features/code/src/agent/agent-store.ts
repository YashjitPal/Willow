/**
 * The Agent tool's harness activity store.
 *
 * The Code tab already owns the project files, the chat messages, and the
 * preview, so duplicating them here would create two sources of truth for the
 * same thing.
 *
 ...
 * belonging to it.
 *
 * Plus the two flags that decide whether any of this runs at all: `agentEngaged`
 * (the Tools-menu selection) and `ultraEngaged` (its effort rung).
 */
...
/**
 * Whether the Agent tool is selected.
 *
 * This is the master switch. Nothing in this folder runs unless it is on: the
 * sidebar routes to the legacy generation loop otherwise, and the harness's
 * composer affordances — slash commands, the Ultra effort rung — stay hidden.
 *
 * It mirrors `selectedToolId` in the two composers rather than replacing it.
 * The Tools menu is still what the user drives, and tools stay single-select;
 * the mirror is what lets a pick made on the landing screen decide how the
 * *opening* turn runs, since the two composers are separate components with
 * separate tool state.
 */
const AGENT_KEY = 'willow:code:agent-tool';

export const agentEngaged = atom<boolean>(readStoredFlag(AGENT_KEY));

export function setAgentEngaged(engaged: boolean): void {
  agentEngaged.set(engaged);
  writeStoredFlag(AGENT_KEY, engaged);
}
 ...
 * over. Only meaningful while `agentEngaged` is on — the legacy loop has no
 * concept of a Codex effort ladder, so both composers hide the row otherwise.
 */
const ULTRA_KEY = 'willow:code:ultra';

export const ultraEngaged = atom<boolean>(readStoredFlag(ULTRA_KEY));

export function setUltraEngaged(engaged: boolean): void {
  ultraEngaged.set(engaged);
  writeStoredFlag(ULTRA_KEY, engaged);
}

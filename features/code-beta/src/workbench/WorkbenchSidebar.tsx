...
// `runComputerUseTest` is gone with the separate test mode. The preview is now
// driven by the harness's `computer_use` tool — see harness-bridge.ts.
import { sandpackStore } from '../runtime/sandpack/sandpack-store';
// `parseAIResponse` and `BOLT_SYSTEM_PROMPT` are gone with the legacy loop: the
// Codex harness writes its own prompt and applies V4A patches, so there is no
// artifact envelope left to parse. `parseResponseForDisplay` stays — it still
// formats prose for the transcript.
import { workbenchStore, parseResponseForDisplay, type ChatSegment } from '../runtime/sandpack/index';
...
// ── Code Beta ────────────────────────────────────────────────────────────
// The Codex harness replaces this sidebar's own generation loop, and its tool
...
const Sidebar: React.FC<SidebarProps> = ({ width, isCollapsed, onToggle, prompt, initialAttachments, activeTab, onTabChange, isChatMode, onHomeClick, modelConfig, setModelConfig, selectedModelId, setSelectedModelId, isResizing, projectName, isProjectPromoted = true, isGeneratingName, onSettingsClick, onProjectHydrated }) => {
   ...
   */
  const slashMatches = matchSlashCommands(promptValue);
  ...
  interface ChatMessage {
    ...
    /**
     * Code Beta only: the Codex harness turn that produced this message.
     *
     * The harness keeps its tool calls and sub-agents in `code-beta-store`
     * rather than on the message, because they stream in while the message body
     * is still empty. This id is the join between the two.
     */
    codexTurnId?: string;
  ...
  // Code changes are now determined from the harness's own edit calls rather
  // than by pattern-matching the prose — see `turnCalls` at the end of a turn.
  ...
  const handleSendMessage = async (text: string) => {
    ...
    // Code Beta: the test branch is gone. Testing is `computer_use`, a harness
    // tool the agent reaches for on its own, rather than a separate mode with
    // its own system prompt and its own model call.
    if (activeTab === 'canvas-screens') {
  ...
  const startAiGeneration = async (text: string, history: AiChatMessage[], uiAlreadyStarted: boolean, currentAttachments: { type: 'image' | 'text' | 'file'; mimeType: string; data: string; name?: string }[] = [], imageAssetPaths: { name: string; path: string; dataUrl: string }[] = []) => {
    ...
    // ── Code Beta: the Codex harness runs this turn ──────────────────────
    //
    // Everything that used to live here — the bolt system prompt, the codebase
    // context block, the streaming message parser — belongs to Willow's own
    // harness and is replaced wholesale. The harness builds its own prompt from
    // the vendored Codex text, sends a file manifest rather than the whole
    // codebase, and applies edits as V4A patches instead of whole-file rewrites.
    //
    // See features/code-beta/src/harness/AGENTS.md.
    ...
    // arrives here and everything else lands in the activity store.
    //
    // Named to avoid shadowing the `history: AiChatMessage[]` parameter, which
    // is the legacy loop's shape and is not what the harness consumes.
    ...
    await runCodexTurn({
      ...
      onText: (chunk) => {
        ...
        // The first token ends the thinking phase, exactly as before.
  ...
  // Code Beta: `startTestGeneration` is deleted.
  //
  // It ran a second, separately-prompted agent loop against the preview via
  // `runComputerUseTest`. That capability is now `computer_use`, a tool the
  // harness owns, so the agent decides when to check its own work instead of
  // the user first flipping the composer into a test mode.
  ...
  /*
   * Code Beta: `Plan` and `Test` are gone.
   *
   * Both are harness tools now — `update_plan` and `computer_use` — so a menu
   * entry that flips the whole composer into a separate prompted mode is a
   * second way to do the same thing, with different behaviour. The agent
   * decides when to plan, and "check the counter increments" reaches
   * `computer_use` without a mode switch.
   *
   * The rest stay: Image, Annotate and Visual Edits are *input* modes that feed
   * context into the composer, not capabilities the agent invokes. Design has
   * its own canvas and its own generation path.
   */
  const TOOLS = [
        ...
        <div className="bg-[#1c1c1c] pointer-events-auto">
          ...
          <div className="px-[14px] pb-4 pt-4">
            ...
            <div className="bg-[#27272a] rounded-[26px] p-3.5 relative flex flex-col shadow-lg border border-white/5">
               ...
               {/*
                 * Code Beta: the slash-command menu.
                 *
               ...
               <textarea
                  ...
                  onKeyDown={(e) => {
                    // Code Beta: the slash menu claims the arrow keys, Tab and
                    // Enter while it is open, so a command can be picked
                    // without the message being sent underneath it.

 ...
 *
 * Upstream widens the Plan-only rule when `Feature::DefaultModeRequestUserInput`
 * is enabled, rather than replacing it:
 *
 *     mode.allows_request_user_input()
 *       || (features.enabled(DefaultModeRequestUserInput) && *mode == Default)
 *
 * The description the model reads is generated from whatever this returns, so
 * turning the flag on changes the sentence to name both modes — which is how a
 * model in Default mode learns the call will now succeed.
 */
export const requestUserInputModes = (askOutsidePlanMode = false): ModeKind[] =>
  COLLABORATION_MODES.filter(
    (mode) => allowsRequestUserInput(mode) || (askOutsidePlanMode && mode === 'default'),
  );

 ...
 * Agent Builder backend (services/agent-builder).
 *
 * Responsibilities:
...
export interface AgentBuilderBackend {
  ...
  submitClientToolResult: (approvalId: string, result: import('./agent-builder').JsonValue) => Promise<void>;
...
export function useAgentBuilderBackend(
  ...
  const submitClientToolResult = useCallback(async (approvalId: string, result: import('./agent-builder').JsonValue) => {
    const runId = runState.get().runId;
    if (!runId) return;
    try {
  ...
  const importWorkflowJson = useCallback(async (text: string) => {
    codeModal.set({ open: true, loading: true, format: 'json', code: '', bundle: null, error: null });
    try {
      const artifact = JSON.parse(text) as import('./agent-builder').PortableWorkflow;

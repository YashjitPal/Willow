...
```
Willow Studio (Vite, :3000) ──HTTP/SSE──▶ Agent Builder backend (:8787) ──▶ Gemini / OpenAI / Anthropic
                                             │        │       │
                                          SQLite    MCP     vector
...
### A) Inside Willow Studio (one command — recommended for dev)

The backend is mounted as **Vite dev middleware**, so Studio's own dev server
hosts the API in-process:

```bash
npm run dev          # from the repo root: serves the app AND the API on http://localhost:3000
```
...
(Wired via `agentBuilderBackend()` in [apps/studio/vite.config.ts](../../apps/studio/vite.config.ts),
which imports [`src/vite-middleware.ts`](src/vite-middleware.ts). Only active in
`vite dev` — production deploys the backend separately.)

### B) Standalone (production / running it on its own)

This service is its own npm package with its own `node_modules`, separate from
the single dependency set the browser side shares at the repo root:

```bash
npm install --prefix services/agent-builder   # first time only
npm run agent-builder:start                   # http://127.0.0.1:8787
npm run agent-builder:test                    # 542 tests
npm --prefix services/agent-builder run demo  # seeds 3 example workflows, runs one live (no API keys needed)
```

To build Studio and serve it with the API from one process, without Vite or
dotenv, run from this directory:

```bash
npm run start:studio
```

That runs Studio's esbuild-only `build:no-env` script, then serves
`../../apps/studio/dist` with SPA routing at `http://127.0.0.1:8787` while
preserving `/api/v1/*` and realtime WebSocket routes. For another build
directory, use `npm start -- --static-dir /absolute/path/to/dist` or set
`AGENT_BUILDER_STATIC_DIR` in the process environment.

To point Studio at a standalone backend instead of the middleware, set
`VITE_AGENT_BUILDER_URL=http://127.0.0.1:8787` in Studio's env.
...
```ts
import { AgentBuilderClient } from '@agentbuilder';
...
client/index.ts       typed SDK for Willow Studio (aliased as `@agentbuilder`)
test/                 542 tests (unit + engine + HTTP + MCP end-to-end)
